...
s_rho ROMS output. Box-average variant of ../plot_cs_diag_totc.py.
...
Total phytoplankton carbon (DIATC + DIAZC + SPC) version.
Output: ./figs/snapshots/cs_diag_totc_box_allscen-<ts|tn>-YYYY-MM-DD-HH.png
"""
...
VARS     = ['DIATC', 'DIAZC', 'SPC']   # summed to give total phytoplankton C
VAR_SRC  = 'bgc'
VAR_CMAP = cmocean.cm.algae
VMIN     = 0
VMAX     = 40
VAR_LABEL = r'Total phyto C (mmol C m$^{-3}$)'
...
for hf in range(n_files):
    ...
    for t_i in range(his_time.shape[0]):
        ...
        # skip if all transect PNGs for this time step already exist
        fnames = [f'{SAVEPATH}cs_diag_totc_box_allscen-{TRANSECT_NAMES[ti]}-{dstr}.png'
                  for ti in range(len(TRANSECTS))]
        if all(os.path.exists(f) for f in fnames):
            print(f'  {dstr} already done, skipping')
            continue
        ...
        for name, (hisnc, varnc) in ncs.items():
            ...
            var3d = sum(np.squeeze(np.array(varnc[v][t_i])) for v in VARS) * mask_plot
        ...
        for ti, tr in enumerate(TRANSECTS):
            fname = f'{SAVEPATH}cs_diag_totc_box_allscen-{TRANSECT_NAMES[ti]}-{dstr}.png'

...
s_rho ROMS output. Box-average variant of ../plot_cs_diag.py: instead of
sampling a single line, averages over a perpendicular box straddling the
transect (see cs_boxavg.py for why -- fixed-depth-grid averaging, not
s-level averaging). Saves one PNG per time step.
...
Output: ./figs/snapshots/cs_diag_box_<SCENARIO>-YYYY-MM-DD-HH.png
"""
...
VAR      = 'w'       # variable name — HIS or BGC
VAR_SRC  = 'his'       # 'his' or 'bgc'
VAR_CMAP = cmocean.cm.balance
VMIN     = -1E-2
VMAX     = 1E-2
VAR_LABEL = r'w (m s$^{-1}$)'
...
for hf in range(n_files):
    ...
    for t_i in range(his_time.shape[0]):
        ...
        for ti, tr in enumerate(TRANSECTS):
            fname = f'{SAVEPATH}cs_diag_box_allscen-{TRANSECT_NAMES[ti]}-{dstr}.png'

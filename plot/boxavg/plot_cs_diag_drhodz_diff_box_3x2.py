...
zsliced his output, laid out as a 3x2 grid. Same box-averaged computation as
plot_cs_diag_drhodz_diff_box.py (identical cache files, reused here directly
-- this script only changes the plot layout), grid/colorbar layout copied
from ../plot_cs_diag_drhodz_diff.py.
...
scenario's time-mean drho/dz DIFFERENCED against the base case. drho/dz has
no single-variable sibling script to copy a fixed raw range from, so the raw
panel's range is derived from the base case itself (2nd/98th percentile over
the plotted depth window). Two colorbars: one for the raw panel (horizontal,
ticks+label above so they don't run into the panel below), one shared across
the 5 diff panels.

Output: ./figs/cs_diag_drhodz_diff_box_3x2_<ts|tn|mid>.png (one per transect)
Cache:  ./figs/cs_diag_drhodz_box_cache_<scen>_<ts|tn|mid>.npz (shared with
        plot_cs_diag_drhodz_diff_box.py -- same underlying field, different
        plot only)
"""
...
def drhodz_cache_path(scen, tr_name):
    # same cache naming as plot_cs_diag_drhodz_diff_box.py -- the underlying
    # box-averaged, time-averaged drho/dz doesn't depend on plot layout, so
...
for ti, tr in enumerate(TRANSECTS):
    fname = f'{SAVEPATH}cs_diag_drhodz_diff_box_3x2_{tr["name"]}.png'

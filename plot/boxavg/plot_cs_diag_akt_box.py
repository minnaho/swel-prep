...
s_rho/s_w ROMS output. Box-average variant of ../plot_cs_diag_akt.py.
...
Vertical thermal conductivity coefficient (Akt) version. Akt lives on s_w
(101 levels, cell interfaces), not s_rho — depths are reconstructed with
get_zw_zeta (w-point depths) instead of plot_cs_diag_rho_box.py's
get_zr_zeta. cs_boxavg's boxavg_section is agnostic to the vertical
dimension length, so no other change is needed for the s_w grid.

Output: ./figs/snapshots/cs_diag_akt_box_allscen-<ts|tn>-YYYY-MM-DD-HH.png
"""
...
VAR      = 'Akt'     # variable name — HIS or BGC
VAR_SRC  = 'his'       # 'his' or 'bgc'
VAR_CMAP = 'viridis'
VMIN     = 1e-5
VMAX     = 1e-1
VAR_LABEL = r'$K_t$ (m$^2$ s$^{-1}$)'
VAR_NORM  = LogNorm(vmin=VMIN, vmax=VMAX)

# Isopycnal contour overlay (sigma-t levels). rho lives on s_rho, not Akt's
# s_w grid, so its own zr3d (get_zr_zeta) is reconstructed separately from
# Akt's zw3d below.
...
for hf in range(n_files):
    ...
    for t_i in range(his_time.shape[0]):
        ...
        for name, (hisnc, varnc) in ncs.items():
            ...
            zw3d  = depths.get_zw_zeta(hisnc, grdnc, zeta)
            zr3d  = depths.get_zr_zeta(hisnc, grdnc, zeta)
            var3d = np.squeeze(np.array(varnc[VAR][t_i])) * mask_plot
            rho3d = (np.squeeze(np.array(hisnc['rho'][t_i])) + ISO_RHO_OFF) * mask_plot
            scen_data[name] = (zw3d, var3d, zr3d, rho3d)

        # one figure per transect
        for ti, tr in enumerate(TRANSECTS):
            fname = f'{SAVEPATH}cs_diag_akt_box_allscen-{TRANSECT_NAMES[ti]}-{dstr}.png'
            ...
            for ax, name in zip(axes, SCENARIOS):
                zw3d, var3d, zr3d, rho3d = scen_data[name]
                var_plot = cb.boxavg_section(var3d, zw3d, tr)

...
s_rho ROMS output. Box-average variant of ../plot_cs_diag_rho.py.
...
Density (rho) version.
Output: ./figs/snapshots/cs_diag_rho_box_allscen-<ts|tn>-YYYY-MM-DD-HH.png
"""
...
VAR      = 'rho'     # variable name — HIS or BGC
VAR_SRC  = 'his'       # 'his' or 'bgc'
VAR_CMAP = cmocean.cm.dense
VMIN     = 24
VMAX     = 26.0
VAR_LABEL = r'$\rho - 1000$ (kg m$^{-3}$)'
...
# Isopycnal contour overlay (sigma-t levels) -- drawn straight off this
# panel's own var_plot, since VAR is already rho
...
for hf in range(n_files):
    ...
    for t_i in range(his_time.shape[0]):
        ...
        for name, (hisnc, varnc) in ncs.items():
            ...
            scen_data[name] = (zr3d, var3d)

        # one figure per transect
        for ti, tr in enumerate(TRANSECTS):
            fname = f'{SAVEPATH}cs_diag_rho_box_allscen-{TRANSECT_NAMES[ti]}-{dstr}.png'

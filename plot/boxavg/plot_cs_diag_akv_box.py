...
s_rho/s_w ROMS output. Box-average variant of ../plot_cs_diag_akv.py.
...
Vertical viscosity coefficient (Akv) version. Akv lives on s_w (101
levels, cell interfaces), not s_rho — depths are reconstructed with
...
Output: ./figs/snapshots/cs_diag_akv_box_allscen-<ts|tn>-YYYY-MM-DD-HH.png
"""
...
VAR      = 'Akv'     # variable name — HIS or BGC
...
# Isopycnal contour overlay (sigma-t levels). rho lives on s_rho, not Akv's
...
for hf in range(n_files):
    ...
    for t_i in range(his_time.shape[0]):
        ...
        for ti, tr in enumerate(TRANSECTS):
            fname = f'{SAVEPATH}cs_diag_akv_box_allscen-{TRANSECT_NAMES[ti]}-{dstr}.png'

"""
KE spectra at 10 m depth, from zsliced his output.

Same complex-velocity FFT approach as calc_ke_surf.py, but the surface
velocity there (raw s_rho top level, always valid in water) is replaced
here by u/v read off the zsliced z_mc60_his.*.nc files at the fixed
z-level closest to -10 m (index 10 on the 157-level, 1 m-spaced-near-
surface zslice depth grid -- z_mc60_his depth[10] == -10.0 exactly).
...
Below-bathymetry fill values (cells shallower than 10 m, where the
...
Output: ./ke_spectra_10m_comparison.npz
"""
...
out_path = './ke_spectra_10m_comparison.npz'
dt_hours = 1.0
...
# time steps per file, and the zslice depth index closest to -10 m, from
...
DEPTH_IDX = int(np.argmin(np.abs(depth_1d - (-10.0))))
...
def uv_rho_at_depth(f, depth_idx, rotate=False):
    ...
    calc_ke_10m.py itself calls with rotate=False, same as calc_ke_surf.py.
...
def calculate_dataset_spectra(file_list, name):
    ...
    # NaN (below-bathymetry-at-10m) cells propagate through mean/FFT and

"""
KE energy cascade at 10 m depth -- same folded-spectrum / power-law-fit
approach as plot_energy_cascade.py, reading from calc_ke_10m.py's
ke_spectra_10m_comparison.npz instead of the surface ke_spectra_comparison.npz.
"""
...
data      = np.load('./ke_spectra_10m_comparison.npz')
...
def make_energy_cascade_plot(scen_keys, out_path):
    ...
    # not the 10m data) so the two plots are directly comparable.
...
make_energy_cascade_plot(SCEN_KEYS, './figs/energy_cascade_10m.png')

make_energy_cascade_plot(
    ['tidesnowec', 'notidesnowec', 'ampwec'],
    './figs/energy_cascade_10m_notidesnowec_group.png',
)

make_energy_cascade_plot(
    ['tideswec', 'notideswec', 'tidesampwec'],
    './figs/energy_cascade_10m_wec_group.png',
)

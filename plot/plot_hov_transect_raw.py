...
plot_cs_diag*.py, computed directly from RAW native s_rho ROMS output
(no z-slicing). Depths are reconstructed per timestep via ROMS_depths
(get_zw_zeta) since sigma levels move with the free surface.

s_rho convention: index 0 = seafloor, index -1 (last) = surface. This
makes "surface" and "bottom" trivial single-level slices; "avg" is the
Hz-weighted mean over the full water column (no depth_lim cutoff, unlike
the z-sliced sibling script).

For each variable, three Hovmöllers are produced:
  avg  — thickness-weighted full-water-column average
  surf — value at the surface (top s_rho level)
  bot  — value at the seafloor (bottom s_rho level)

Variables:
  N²      — Brunt-Väisälä frequency squared from raw rho
  w       — vertical velocity
  NO3     — nitrate
  O2      — dissolved oxygen
...
Time is read from each file's ocean_time (seconds since 1995-01-01,
matching the convention used by plot_cs_diag*.py). his/bgc files within
a scenario are paired positionally by sorted filename order (guards
against scenarios with a differing bgc file count, e.g. tidesnowec has
2 extra trailing bgc files with no his counterpart).

See plot_hov_transect_zslice.py for the z-sliced (fixed-depth) version
of this same set of plots.

Output: ./figs/hov_transect_raw/hov_{var}_{avg,surf,bot}_<ts|tn>.png
"""
...
# ---------------------------------------------------------------------------
GRD      = 'mc60_grd.nc'
SAVEPATH = './figs/hov_transect_raw/'
...
def src_glob(root, kind):
    ...
    (ampwec is flat; the other four scenarios use his/ + bgc/ subdirs)."""

...
get_zw_zeta (w-point depths) instead of plot_cs_diag_rho.py's get_zr_zeta.

Output: ./figs/snapshots/cs_diag_akt_allscen-<ts|tn>-YYYY-MM-DD-HH.png
"""
...
def interp_section(field3d, coords, mask_t):
    """Interpolate a 3D (s_w, eta, xi) field along a transect. Returns (s_w, N_PTS)."""
    n_s = field3d.shape[0]
...
for hf in range(n_files):
    ...
    for t_i in range(his_time.shape[0]):
        ...
        for ti, tr in enumerate(TRANSECTS):
            ...
            fname = f'{SAVEPATH}cs_diag_akt_allscen-{TRANSECT_NAMES[ti]}-{dstr}.png'

"""
RMSE(depth) profile of normalized relative vorticity (zeta/f) offshore
(eta_rho index > 186, h > 100 m), each line a comparison against the
notidesnowec base case -- see postprocessing/calc_vort_rmse_wec_offshore.py
for the calculation. Vorticity counterpart of plot_w_rmse_wec_offshore_std.py
-- same plot, same three comparisons, just the field differs (zeta/f instead
of w). Answers: does WEC change the strength of tidal-bore-driven vorticity,
beyond the WEC-driven vorticity signature that already exists without tides?
...
postprocessing/calc_vort_std.py -- on the SAME x-axis as RMSE (same
dimensionless zeta/f units). RMSE alone can't tell "more variable vorticity"
apart from "same variability, phase-shifted relative to the base"; both
inflate RMSE identically. Where RMSE is large but Delta-std ~ 0 at a given
depth, the difference there is redistribution/phase, not a change in
variability magnitude. Delta-std can be negative (RMSE cannot), so a
vertical zero line is drawn and the x-axis now generally extends below zero.
See plot_vort_rmse_wec_offshore.py for the RMSE-only version of this plot
(unmodified).
...
Reads: ../postprocessing/vort_rmse_wec_offshore.npz, ../postprocessing/vort_std.npz
  (std overlay is skipped with a warning if vort_std.npz doesn't exist yet)
Output: ./figs/vort_rmse_wec_offshore_std.png
"""
...
NPZ = '../postprocessing/vort_rmse_wec_offshore.npz'
STD_NPZ = '../postprocessing/vort_std.npz'
SAVEPATH = './figs/'
REGION = 'offshore'
...
if os.path.exists(STD_NPZ):
    ...
    print(f'WARNING: missing {STD_NPZ} -- run calc_vort_std.py first; '
...
ax.set_xlabel(r'RMSE and $\Delta$std of $\zeta/f$ vs no tides, no WEC')
...
fname = f'{SAVEPATH}vort_rmse_wec_offshore_std.png'

"""
h>100m-restricted version of plot_wno3_flux_100m.py's NO3_20m panel -- same
4-panel layout (time series, box plot, envelope, PDF) for w'NO3' flux at
20m, but reading from the offshore-only (h>100m) npz files written by
calc_wno3_flux_20m_offshore.py instead of the shelf-only (h<=100m) ones.

Scoped to NO3_20m only: unlike plot_wno3_flux_100m.py (which also covers
NO3 at 10m and ptrace/rtrace at 10m/20m), no offshore counterpart exists yet
for calc_wno3_flux_10m_100m.py or the ptrace/rtrace calc scripts -- this
script only plots what calc_wno3_flux_20m_offshore.py actually produces.
Structured as a single-entry TRACERS dict (not a bare script) so adding
those siblings later is a one-entry addition, same as
plot_wno3_flux_100m.py's own pattern.

Output: ./figs/wno3_flux_20m_offshore.png
"""

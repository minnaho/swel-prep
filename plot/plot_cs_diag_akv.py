...
Vertical viscosity coefficient (Akv) version. Akv lives on s_w (101 levels,
cell interfaces), not s_rho — depths are reconstructed with get_zw_zeta
(w-point depths) instead of plot_cs_diag_rho.py's get_zr_zeta.

Output: ./figs/snapshots/cs_diag_akv_allscen-<ts|tn>-YYYY-MM-DD-HH.png
"""
...
for hf in range(n_files):
    ...
    for t_i in range(his_time.shape[0]):
        ...
        for ti, tr in enumerate(TRANSECTS):
            ...
            fname = f'{SAVEPATH}cs_diag_akv_allscen-{TRANSECT_NAMES[ti]}-{dstr}.png'

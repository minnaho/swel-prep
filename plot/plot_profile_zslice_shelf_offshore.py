...
combining the h<=100m (shelf) and h>100m (offshore) domains into one figure
per variable (analog of plot_profile_zslice_100m.py, but side-by-side with
its offshore counterpart from profile_zslice_par_offshore.py).
...
Six scenarios are overlaid as colored lines on each mean panel.

Panels 1-2 and 3-4 use separate y-axis limits (DEPTH_YLIM_SHELF /
DEPTH_YLIM_OFFSHORE) -- shelf data never extends below -100m while offshore
data extends to -1980m, so sharing one y-axis across all four panels would
leave the shelf panels mostly empty. Similarly, x-limits are region-specific
(XLIM_SHELF / XLIM_OFFSHORE) since e.g. offshore NO3 reaches ~32 vs the
shelf's capped [0, 20] display range.

Output: ./figs/profiles/profile_zslice_shelf_offshore_<var>.png
"""
...
for var in VARS_HIS + VARS_BGC + VARS_DIA + VARS_AK:
    ...
    plt.tight_layout()
    out = f'{SAVEPATH}profile_zslice_shelf_offshore_{var}.png'

...
mcnc = Dataset('../tides/mc60/mc60_grd_fill.nc','r')
...
mc_p = np.array(mcnc.variables['pipe_flux'])
mc_rj, mc_ri = np.where(mc_r>0)
mc_pj, mc_pi = np.where(mc_p>0)
...
fig,ax = plt.subplots(1,1,subplot_kw=dict(projection=ccrs.PlateCarree()))
p_plot = ax.pcolormesh(mc_lon,mc_lat,mc_h,transform=ccrs.PlateCarree(),cmap=cmocean.cm.deep,vmin=0,vmax=200)
#ax.add_feature(coast_10m,facecolor='None',edgecolor='k')
#ax.contour(mc_lon,mc_lat,mc_h,[2])
ax.contour(mc_lon,mc_lat,mc_mask,[0])
#ax.contour(mc_lon,mc_lat,mc_h,[30])
#ax.contour(mc_lon,mc_lat,mc_h,[10])
...
ax.scatter(mc_lon[mc_pj,mc_pi],mc_lat[mc_pj,mc_pi],label='pipes',color='orange')
ax.scatter(mc_lon[taper10j,taper10i],mc_lat[taper10j,taper10i],label='taper10',color='pink')

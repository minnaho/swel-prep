...
_fix version: divides stored flux (mmol/s) by dz*dy_face to get flux per
unit area (mmol m^-2 s^-1), removing the artifact where 30-m cells below
300 m appear ~6x stronger than 5-m cells above.
"""
...
CACHE   = f'./figs/offshore_flux_hov_time_zslice_fix_cache_{TRACER}.npz'
...
if os.path.exists(CACHE):
    ...
    for name, _ in scenario_rows:
        ...
        F_td    = np.nanmean(flux_pa, axis=2)     # (time, n_z) — collapse alongshore
...
out = f'./figs/offshore_flux_hov_time_zslice_fix_{TRACER}.png'

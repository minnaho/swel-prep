"""
Parent (smode200) vs child (mc60) w'NO3' time series at 10 m.
...
Averaging-window note: the child's w'NO3' (calc_wno3_flux_10m.py) removes a
...
def load_child(name):
    ...
    if not os.path.exists(path):
        print(f'  WARNING: {path} not found -- run calc_wno3_flux_10m.py first. '
              f'Skipping {name}.')
        return None

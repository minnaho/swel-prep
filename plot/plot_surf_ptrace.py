...
savepath = './figs/snapshots/ptrace/'
...
for hf in range(len(hisfiles1)):
    ...
    for t_i in range(his_time.shape[0]):
        ...
        out_file = savepath+'surf_ptrace-'+str(dt0.year)+'-'+'%02d'%dt0.month+'-'+'%02d'%dt0.day+'-'+'%02d'%dt0.hour+'.png'
        ...
        tracer4mask = np.squeeze(hisnc4.variables['ptrace'])[t_i,-1]*masknc # get surface 

function r2r_make_inisedbgc(par_grd,par_data,chd_grd, chd_data,   ...
...
%      filefrc = BGC_INI.bgc_frcini{BGC_INI.tracer_roms(trc)} ; 
      ...
      var = squeeze(ncread(par_data,trcname  ,[imin jmin tind],[imax-imin+1,jmax-jmin+1 1]));
      var = fillmask(var',1,masks,nnel);
      var = double(var);
      ini_var = sum(coef2d.*var(elem2d), 3); 
      
      disp(' Writing ini file , for netcdf')
      ncwrite(chd_data,trcname  ,ini_var',[icb jcb 1]);
      end
      end

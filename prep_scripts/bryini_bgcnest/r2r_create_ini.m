function r2r_create_ini(inifile,gridfile,N,chdscd,makebgc,BGC_INI,bgctracers_list,do_slow)
%
%   Input: 
% 
...
%
if do_slow==1
nccreate(inifile,'u_slow','Dimensions',{'xi_u',L,'eta_u',Mp,'s_rho',N,'time',1},'datatype','single');
ncwriteatt(inifile,'u_slow','long_name','time filtered u');
ncwriteatt(inifile,'u_slow','units','m/s');
%
nccreate(inifile,'v_slow','Dimensions',{'xi_v',Lp,'eta_v',M,'s_rho',N,'time',1},'datatype','single');
ncwriteatt(inifile,'v_slow','long_name','time filtered v');
...
ncwriteatt(inifile,'p_slow','long_name','time filtered p');
ncwriteatt(inifile,'p_slow','units','Pa??');
end
%

function r2r_make_ini(par_grd,par_data,chd_grd, chd_data,   ...
                      chdscd,   parscd, scoord_switch_p, scoord_switch_c, do_slow );
%--------------------------------------------------------------
%
...
%      disp('WARNING chilg grid angle converted to radians from degrees')
%      angc = angc*pi/180.0 ; 
      cosc  = cos(angc);
      sinc  = sin(angc);
...
%     save 'testing' 'A' 'var'
      ...
      if do_slow
      disp(['--- p_slow'])
      var = squeeze(ncread(par_data,'p_slow'  ,[imin jmin 1 tind],[imax-imin+1,jmax-jmin+1 inf 1]));
      var = permute(var,[3 2 1]);
      var = fillmask(var,1,masks,nnel);
      var = double(var);
      ini_pslow = reshape(A*reshape(var,Np*Mp*Lp,1),Nc,Mc,Lc);
      end
    ...
    % Get barotropic velocity
      ...
      if do_slow
      disp('--- slow baroclinic velocity');
      usd = squeeze(ncread(par_data,'u_slow'  ,[imin jmin 1 tind],[imax-imin,jmax-jmin+1 inf 1]));
      vsd = squeeze(ncread(par_data,'v_slow'  ,[imin jmin 1 tind],[imax-imin+1,jmax-jmin inf 1]));
      usd = permute(usd,[3 2 1]);
      vsd = permute(vsd,[3 2 1]);
      usr = u2rho_3d(usd);
      vsr = v2rho_3d(vsd);
    % Rotate to north
      uss = zeros(Np,Mp,Lp);
      vss = zeros(Np,Mp,Lp);
      for k = 1:Np
        uss(k,:,:) = squeeze(usr(k,:,:)).*coss - squeeze(vsr(k,:,:)).*sins;
        vss(k,:,:) = squeeze(vsr(k,:,:)).*coss + squeeze(usr(k,:,:)).*sins;
      end
      uss = fillmask(uss, 0, masks, nnel);
      vss = fillmask(vss, 0, masks, nnel);
      uss = double(uss);
      vss = double(vss);
      usd = reshape(A*reshape(uss, Np*Mp*Lp,1), Nc,Mc,Lc);
      vsd = reshape(A*reshape(vss, Np*Mp*Lp,1), Nc,Mc,Lc);
    % Rotate to child orientation
      uss = zeros(Nc, Mc, Lc);
      vss = zeros(Nc, Mc, Lc);
      for k=1:Nc
        uss(k,:,:) = squeeze(usd(k,:,:)).*cosc + squeeze(vsd(k,:,:)).*sinc;
        vss(k,:,:) = squeeze(vsd(k,:,:)).*cosc - squeeze(usd(k,:,:)).*sinc;
      end
    % Back to staggered locations
      us = 0.5*(uss(:,:,1:Lc-1) + uss(:,:,2:Lc));
      vs = 0.5*(vss(:,1:Mc-1,:) + vss(:,2:Mc,:));
      end
    ...
    % Zero-ing out the mask
      for k = 1:Nc
        ...
        if do_slow
        ini_pslow(k,:,:) = squeeze(ini_pslow(k,:,:));
        us(k,:,:)        = squeeze(us(k,:,:)).*umask;
        vs(k,:,:)        = squeeze(vs(k,:,:)).*vmask;
        end
      end
...
%      ncwrite(chd_data,'zeta'  ,zetac'  ,[icb jcb 1]  );
      ...
      if do_slow
      ini_pslow = permute(ini_pslow,[3 2 1]);
      ncwrite(chd_data,'p_slow'  ,ini_pslow,[icb jcb 1 1]);
      us = permute(us,[3 2 1]);
      ncwrite(chd_data,'u_slow'     ,us       ,[icb jcb 1 1]);
      vs = permute(vs,[3 2 1]);
      ncwrite(chd_data,'v_slow'     ,vs       ,[icb jcb 1 1]);
      end

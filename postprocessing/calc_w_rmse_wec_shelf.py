...
the strength of tidal-bore-driven w on the shelf.
...
elsewhere in plot/ -- not a lat/lon bound).
...
Usage:
  python -u calc_w_rmse_wec_shelf.py [--nfiles N]
...
magnitude as calc_w_rms_pycnocline.py's per-scenario cost. Use --nfiles for
a quick smoke test before committing to the full run.

Output: ./w_rmse_wec_shelf.npz

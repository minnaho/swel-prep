"""
Fully parallel version of profile_zslice.py — one job per scenario per filetype
so all 15 can run simultaneously.

Usage:
  # launch all in parallel:
  for scen in tideswec tidesnowec notidesnowec notideswec ampwec tidesampwec; do
    python -u profile_zslice_par.py $scen his > log_${scen}_his.txt 2>&1 &
    python -u profile_zslice_par.py $scen bgc > log_${scen}_bgc.txt 2>&1 &
    python -u profile_zslice_par.py $scen dia > log_${scen}_dia.txt 2>&1 &
  done

  # once all 15 are done:
  python -u profile_zslice_par.py --merge

Output per job:  zslice_profiles_<scenario>_<filetype>.npz
...
if args.merge:
    ...
    for scen in SCENARIOS:
        for ft in FILETYPES:
            fname = f'zslice_profiles_{scen}_{ft}.npz'
            try:
...
fname = f'zslice_profiles_{scen}_{filetype}.npz'
np.savez(fname, **out)
print(f'saved {fname}')

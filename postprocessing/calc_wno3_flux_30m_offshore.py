"""
h>100m-restricted version of calc_wno3_flux_30m.py -- same w'NO3' flux
calculation at the 30 m zslice level, but the horizontal domain is masked to
...
profile_zslice_par_offshore.py (mask[h <= 100] = np.nan applied to the base
mask_rho land mask). Offshore counterpart of calc_wno3_flux_30m_100m.py,
which is the h<=100m (shelf) restriction. Structurally identical to
calc_wno3_flux_20m_offshore.py (TARGET_DEPTH/argmin approach, no
START_TRIM), just at 30 m -- unlike the h<=100m 20m/30m variants, which do
apply START_TRIM=3; kept consistent with the existing offshore sibling
rather than the shelf family here.

Output: wno3_flux_30m_offshore_<scenario>.npz, wno3_env_30m_offshore_<scenario>.npz
"""
...
FILL_THRESH  = 0.9e33
TARGET_DEPTH = -30   # m — select this level from the zslicefull depth array
...
for name, (zslice_dir, orig_his_dir) in scenarios.items():
    ...
    outfile    = f'wno3_flux_30m_offshore_{name}.npz'
    outfileenv = f'wno3_env_30m_offshore_{name}.npz'

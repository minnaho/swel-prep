...
whether WEC changes the strength of tidal-bore-driven vorticity on the
shelf. Vorticity counterpart of calc_w_rmse_wec_shelf.py -- same three
comparisons, same pooled RMSE(depth) definition, same shelf mask; only the
field differs (normalized relative vorticity zeta/f instead of w).
...
load pattern calc_w_rmse_wec_shelf.py uses for `w` -- pyfuncs.rho_uv_angle
...
Usage:
  python -u calc_vort_rmse_wec_shelf.py [--nfiles N]
...
roughly 2x calc_w_rmse_wec_shelf.py's cost since vorticity needs both u and
v where w_rmse only reads w. Use --nfiles for a quick smoke test before
committing to the full run.

Output: ./vort_rmse_wec_shelf.npz
...
def build_mask(eta_rho):
    ...
    return (eta_idx > ETA_MIN) & (h < H_MAX) & (mask_rho == 1)
...
def compute_all_profiles(nfiles=None):
    ...
    print(f'shelf mask: {mask2d.sum()} cells (eta_rho > {ETA_MIN}, h < {H_MAX} m)')
...
if __name__ == '__main__':
    ...
    outfile = 'vort_rmse_wec_shelf.npz'

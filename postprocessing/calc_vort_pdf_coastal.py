...
# restrict to the 10 km coastal band -- cells outside it become NaN so they
# drop out of the PDF the same way land does
coastal_mask = np.array(Dataset(COASTAL_MASK_FILE)['coastal_mask'])
masknc[coastal_mask == 0] = np.nan
...
def process_scenario(name, files):
    ...
    out_path = f'./pdf_vorticity_{name}_coastal.npz'

"""
h<=100m-restricted version of calc_wtrace_flux.py -- same w'ptrace'/w'rtrace'
flux calculation (both tracers, both 10m and 20m zslice levels), but the
horizontal domain is masked to the shelf (h<=100m) before any spatial
statistic (envelope max/min/mean, PDF) is computed, same masking approach as
calc_vort_pdf_100m.py / profile_zslice_par_100m.py (mask[h > 100] = np.nan
applied to the base mask_rho land mask).

Output: w<tracer>_env<suffix>_100m_<scenario>.npz
  e.g. wptrace_env_100m_notidesnowec.npz, wrtrace_env_20m_100m_tidesampwec.npz
"""
...
# calc_wtrace_flux.py, just spatially restricted.
...
# calc_wno3_flux_10m_100m.py/_20m_100m.py
DEPTHS = [(10, ''), (20, '_20m')]
...
for tracer in ['ptrace', 'rtrace']:
    ...
    for depth_idx, suffix in DEPTHS:
        ...
        for name, (zslice_alias, raw_his_dir) in scenarios.items():
            outfileenv = f'w{tracer}_env{suffix}_100m_{name}.npz'

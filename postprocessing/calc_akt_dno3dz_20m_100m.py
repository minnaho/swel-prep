...
flux, Akt * dNO3/dz, at 20 m depth -- diffusive-flux counterpart of
calc_wno3_flux_20m_100m.py's resolved eddy flux w'NO3'. Same 4-scenario set,
same shelf mask (mask[h > 100] = np.nan applied to the base mask_rho land
mask), same ~20-M2-cycle record trim (START_TRIM=3, see that script's
docstring for the exact cycle-count reasoning), and the same output npz
schema (flux file: time_series/ocean_times/bin_centers/pdf/mean_flux; env
file: ts_min/ts_max/ts_mean/ocean_times/bin_centers/pdf/mean_flux) so it's a
drop-in alternate npz source for a plot_wno3_flux_100m.py-style config entry.
...
calc_wno3_flux_20m_100m.py already uses. Both share the exact same fixed
...
50-100), 30m spacing -300 to -1980m (indices 100-156). 20m falls well inside
the 1m-spaced tier, so the centered difference at the two levels bracketing
it (index 19/21 around index 20) is still exact (spans exactly ±1m, no
...
Output: akt_dno3dz_flux_20m_100m_<scenario>.npz, akt_dno3dz_env_20m_100m_<scenario>.npz
"""
...
FILL_THRESH  = 0.9e33
TARGET_DEPTH = -20   # m — select the level bracketing this from the zslicefull depth array
...
# same 4 scenarios as calc_wno3_flux_20m_100m.py
scenarios = {
...
def get_bracket_indices(nc):
    ...
    Exact (no interpolation needed) only because TARGET_DEPTH=-20 falls in
...
for name, (zslice_dir, orig_his_dir) in scenarios.items():
    ...
    outfile    = f'akt_dno3dz_flux_20m_100m_{name}.npz'
    outfileenv = f'akt_dno3dz_env_20m_100m_{name}.npz'

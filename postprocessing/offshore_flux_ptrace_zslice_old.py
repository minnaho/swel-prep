"""
Offshore advective flux of ptrace using z-sliced output.

Reads z_mc60_his.*.nc from /data/project1/minnaho/swel/zslicefull/<scenario>/,
which carry ptrace and u on a fixed uniform z-grid (157 levels, 0 to -1980 m).

F(t, z, j) = -u(t, z, j, i_left-1) * ptrace(t, z, j, i_left) * dy(j) * dz(z, j)
...
is read from the matching original history file (not stored in the z-sliced
file).
...
Output (per scenario) saved as offshore_flux_ptrace_zslice_<scenario>.npz:
    offshore_flux : (time, n_z, n_valid)   mmol s^-1
...
TRACER    = 'ptrace'
OUT_NAME  = 'offshore_flux_ptrace_zslice'
...
def compute_flux(zslice_dir, his_dir):
    ...
    for zf in z_files:
        ...
        flux = (-u_edge * trac_edge
                * dy_face[None, None, :]
                * dz[None, :, :])                            # (t, n_z, n_valid)

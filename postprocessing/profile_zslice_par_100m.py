"""
Fully parallel version of profile_zslice_par.py, restricted to the 100 m
isobath and shallower -- same h<=100m masking approach used in
calc_vort_pdf_100m.py (mask[h > 100] = np.nan), applied to the base
rho/u/v masks before any per-file accumulation, so every output mask
('full', 'coastal') is implicitly an average over depth <= 100 m only.

The h0to50/h50to200/h200p depth-bin masks from profile_zslice_par.py are
dropped here -- they'd otherwise silently re-bin within an already
h<=100m-restricted domain (e.g. 'h50to200' would really only cover
50-100 m, not 50-200 m), which would be misleading.
...
Output of merge: zslice_profiles_100m.npz
                 zslice_profiles_100m_coastal.npz
"""
...
if args.merge:
    ...
    for scen in SCENARIOS:
        for ft in FILETYPES:
            fname = f'zslice_profiles_100m_{scen}_{ft}.npz'
            try:
    ...
    np.savez('zslice_profiles_100m.npz', **out)
    print('saved zslice_profiles_100m.npz')
    ...
    np.savez('zslice_profiles_100m_coastal.npz', **coastal_out)
    print('saved zslice_profiles_100m_coastal.npz')
    raise SystemExit(0)
...
# restrict to areas 100 m and shallower -- deeper cells become NaN so they
...
fname = f'zslice_profiles_100m_{scen}_{filetype}.npz'
np.savez(fname, **out)
print(f'saved {fname}')

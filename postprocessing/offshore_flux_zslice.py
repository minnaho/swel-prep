"""
Offshore advective flux of BGC tracers using z-sliced output.

Reads z_mc60_bgc.*.nc and z_mc60_his.*.nc (u) from
/data/project1/minnaho/swel/zslicefull/<scenario>/, matched by timestamp.
Both carry variables on a fixed uniform z-grid (157 levels, 0 to -1980 m).

F(t, z, j) = -u(t, z, j, i_left-1) * C(t, z, j, i_left)

u is taken directly from the u-face at xi_u = i_left - 1 (western face of the
leftmost band cell).  ocean_time is read from the matching original his file.

Tracers:
  Single-variable: NO3, NH4, O2, DIC, DOC  (one NPZ each)
  Summed groups (summed before the flux multiply):
    TOTC   = SPC + DIATC + DIAZC   (total phytoplankton carbon)
    TOTCHL = SPCHL + DIATCHL + DIAZCHL  (total chlorophyll)

dz (cell thickness) is computed per (z, j) column:
  - Interfaces between levels are midpoints of adjacent z-centers; the top
    interface is fixed at 0, the bottom is extrapolated half a step below
    the deepest level.
  - Interfaces are then clipped to -h(j, i_left) so cells straddling the
    seabed get a partial thickness and cells fully below the seabed get
    dz = 0.  h is sampled at the tracer rho-point (jj, il).

_FillValue entries (1e33) are converted to NaN immediately after reading.
NaN propagates through the flux multiplication so that masked and
below-seabed points remain NaN in the output, distinguishing missing data
from genuine zero flux.

Output (per scenario, per tracer): offshore_flux_zslice_<scenario>_<tracer>.npz
    offshore_flux : (time, n_z, n_valid)   tracer units · m^-2 s^-1  (flux per unit area)
    depth         : (n_z,)                 m, negative downward
    dz            : (n_z, n_valid)         m, bathymetry-clipped thickness
    ocean_time    : (time,)                seconds since 1995-01-01
    eta_idx       : (n_valid,)             int32
    xi_idx        : (n_valid,)             int32
    dy_face       : (n_valid,)             m
    h_edge        : (n_valid,)             m, positive bathymetry at band edge
    tracer_name   : str
"""

import glob
import os
import numpy as np
from netCDF4 import Dataset

GRD       = '../mc60_grd.nc'
MASK_FILE = '../plot/coastal_mask.nc'
OUT_NAME  = 'offshore_flux_zslice'

SINGLE_TRACERS = ['NO3', 'NH4', 'O2', 'DIC', 'DOC']

SUMMED_TRACERS = {
    'TOTC':   ['SPC',   'DIATC',   'DIAZC'],
    'TOTCHL': ['SPCHL', 'DIATCHL', 'DIAZCHL'],
}

ZSLICE_ROOT = '/data/project1/minnaho/swel/zslicefull'

scenarios = {
    'notidesnowec': ('notidesnowec',
                     '/data/project3/minnaho/swel/notides/mc60/nowec/his'),
    'ampwec':       ('notidesampwec',
                     '/data/project3/minnaho/swel/notides/mc60/wec/ampwec/everything'),
    'tidesnowec':   ('tidesnowec',
                     '/data/project3/minnaho/swel/tides/mc60/nowec/output/his'),
    'tidesampwec':  ('tidesampwec',
                     '/data/project3/minnaho/swel/tides/mc60/ampwec/everything'),
}

# tidesampwec's raw source has a trailing 1-timestep file
# (...20190429110056) whose zslice output has no time dimension at all --
# same exclusion as calc_wtrace_flux.py / plot_cs_diag_avg_diff.py
TIDESAMPWEC_EXCLUDE = ('20190429110056',)

# --- coastal band western edge ---
mask = np.array(Dataset(MASK_FILE)['coastal_mask'])
ny, nx = mask.shape
i_left = np.full(ny, -1, dtype=int)
for j in range(ny):
    band = np.where(mask[j, :] == 1)[0]
    if band.size > 0:
        i_left[j] = band.min()

valid   = i_left >= 0
jj      = np.where(valid)[0]
il      = i_left[valid]
n_valid = jj.size
print(f'{n_valid} rows have a coastal-band edge')

grdnc   = Dataset(GRD)
dy_face = 1.0 / np.array(grdnc.variables['pn'])[jj, il]   # (n_valid,)
h_edge  = np.array(grdnc.variables['h'])[jj, il]           # (n_valid,) positive


def _fill_to_nan(arr):
    """Replace _FillValue sentinel (|x| > 1e30) with NaN in a float array."""
    return np.where(np.abs(arr) > 1e30, np.nan, arr)


def compute_dz_2d(depth_vals, h_edge):
    """
    Bathymetry-aware cell thicknesses for a non-uniform z-grid.

    depth_vals : (n_z,)     m, negative downward, cell centers (from file)
    h_edge     : (n_valid,) m, positive bathymetry at the band edge

    Returns dz : (n_z, n_valid) m.
      Cells fully above the seabed: full midpoint-rule thickness.
      Cell straddling the seabed: partial (clipped) thickness.
      Cells fully below the seabed: 0.
    """
    n_z = depth_vals.size
    z_w = np.empty(n_z + 1)
    z_w[0]    = 0.0
    z_w[1:-1] = (depth_vals[:-1] + depth_vals[1:]) / 2.0
    bottom_step = float(depth_vals[-2]) - float(depth_vals[-1])   # positive
    z_w[-1]   = float(depth_vals[-1]) - bottom_step / 2.0

    # Clip interfaces against -h; broadcast over n_valid
    z_w_clipped = np.maximum(z_w[:, None], -h_edge[None, :])   # (n_z+1, n_valid)

    return z_w_clipped[:-1, :] - z_w_clipped[1:, :]   # (n_z, n_valid)


def compute_flux(zslice_dir, his_dir):
    """
    Returns:
        flux_by_tracer : dict {tracer_name: (time, n_z, n_valid) float32}
        depth_vals     : (n_z,) m
        dz             : (n_z, n_valid) m
        ocean_time     : (time,) s
    """
    bgc_files = sorted(glob.glob(os.path.join(zslice_dir, 'bgc', 'z_mc60_bgc.*.nc')))
    his_files = sorted(glob.glob(os.path.join(zslice_dir, 'z_mc60_his.*.nc')))
    bgc_files = [f for f in bgc_files if not any(x in f for x in TIDESAMPWEC_EXCLUDE)]
    his_files = [f for f in his_files if not any(x in f for x in TIDESAMPWEC_EXCLUDE)]
    print(f'  {len(bgc_files)} z-sliced bgc files, {len(his_files)} his files')

    bgc_map = {os.path.basename(f).replace('z_mc60_bgc.', '').replace('.nc', ''): f
               for f in bgc_files}
    his_map = {os.path.basename(f).replace('z_mc60_his.', '').replace('.nc', ''): f
               for f in his_files}
    common = sorted(set(bgc_map) & set(his_map))
    print(f'  {len(common)} matched timestamps')

    with Dataset(his_files[0]) as nc:
        depth_vals = np.array(nc.variables['depth'][:])   # (n_z,) from file
    dz = compute_dz_2d(depth_vals, h_edge)   # (n_z, n_valid)

    all_tracers = list(SINGLE_TRACERS) + list(SUMMED_TRACERS.keys())
    flux_chunks = {tr: [] for tr in all_tracers}
    time_chunks = []

    for ts in common:
        bf = bgc_map[ts]
        zf = his_map[ts]
        hf = os.path.join(his_dir, f'mc60_his.{ts}.nc')

        with Dataset(zf) as nc:
            u_full = _fill_to_nan(np.array(nc.variables['u'][:]))
            u_edge = u_full[:, :, jj, il - 1]   # (t, n_z, n_valid)
            del u_full

        with Dataset(bf) as nc:
            available = set(nc.variables.keys())

            for tr in SINGLE_TRACERS:
                if tr not in available:
                    print(f'    WARNING: {tr} not in {os.path.basename(bf)}, skipping')
                    flux_chunks[tr].append(None)
                    continue
                c_full = _fill_to_nan(np.array(nc.variables[tr][:]))
                c_edge = c_full[:, :, jj, il]   # (t, n_z, n_valid)
                del c_full
                flux = (-u_edge * c_edge)
                flux_chunks[tr].append(flux.astype(np.float32))

            for tr_out, components in SUMMED_TRACERS.items():
                missing = [c for c in components if c not in available]
                if missing:
                    print(f'    WARNING: {missing} not in {os.path.basename(bf)}, '
                          f'skipping {tr_out}')
                    flux_chunks[tr_out].append(None)
                    continue
                c_sum = None
                for comp in components:
                    c_comp = _fill_to_nan(np.array(nc.variables[comp][:]))
                    c_comp = c_comp[:, :, jj, il]   # (t, n_z, n_valid)
                    if c_sum is None:
                        c_sum = c_comp
                    else:
                        c_sum = c_sum + c_comp   # NaN + finite = NaN (propagates)
                    del c_comp
                flux = (-u_edge * c_sum)
                flux_chunks[tr_out].append(flux.astype(np.float32))
                del c_sum

        with Dataset(hf) as his:
            time_chunks.append(np.array(his.variables['ocean_time'][:]))

    ocean_time = np.concatenate(time_chunks)

    flux_by_tracer = {}
    for tr in all_tracers:
        chunks = flux_chunks[tr]
        if any(c is None for c in chunks):
            print(f'  Skipping {tr} — missing from one or more files')
            continue
        flux_by_tracer[tr] = np.concatenate(chunks, axis=0)

    return flux_by_tracer, depth_vals, dz, ocean_time


for name, (zscen, his_dir) in scenarios.items():
    print(f'\nProcessing {name} ...')
    zslice_dir = os.path.join(ZSLICE_ROOT, zscen)
    flux_by_tracer, depth_vals, dz, ot = compute_flux(zslice_dir, his_dir)

    for tr, flux in flux_by_tracer.items():
        out = f'{OUT_NAME}_{name}_{tr}.npz'
        np.savez(out,
                 offshore_flux = flux,
                 depth         = depth_vals,
                 dz            = dz,
                 ocean_time    = ot,
                 eta_idx       = jj.astype(np.int32),
                 xi_idx        = il.astype(np.int32),
                 dy_face       = dy_face,
                 h_edge        = h_edge,
                 tracer_name   = tr)
        print(f'  saved -> {out}  shape {flux.shape}')
